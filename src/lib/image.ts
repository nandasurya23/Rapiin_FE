...
/**
 * Kompresi gambar logo secara lokal menggunakan HTML Canvas dan kembalikan sebagai Blob.
 */
export function compressLogoAsBlob(file: File, maxW = 180, maxH = 180): Promise<Blob> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      const img = new Image();
