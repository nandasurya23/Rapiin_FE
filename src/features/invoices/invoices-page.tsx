...
export function InvoicesPage() {
 ...
 const selectedOrderLink = selectedInvoice ? ROUTES.invoice(selectedInvoice.id) : ROUTES.invoices(business.slug);
   ...
   <PageHeader
    ...
    action={
     <div className="flex flex-wrap gap-2.5 xl:shrink-0">
      ...
      <LinkButton
       href={selectedInvoice ? ROUTES.invoice(selectedInvoice.id) : ROUTES.invoices(business.slug)}
       variant="secondary"
       className="font-bold"
      >
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-white dark:bg-[#101725] shadow-sm p-6 relative overflow-hidden">
         ...
         <div className="mt-5 flex flex-wrap gap-2 border-t border-dashed border-[var(--color-border)] pt-5">
          <WhatsAppButton
           ...
           message={`Halo ${selectedInvoice.customerName}, ini nota untuk ${selectedInvoice.invoiceCode}. Silakan cek preview nota detail di link berikut: ${typeof window !== "undefined" ? window.location.origin : ""}${ROUTES.invoice(selectedInvoice.id)}`}
           label="Kirim WA"
          />
          <Button
           type="button"
          ...
          <LinkButton
           href={ROUTES.invoice(selectedInvoice.id)}
           variant="secondary"
